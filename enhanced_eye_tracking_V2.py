...
class EnhancedEyeTracker:
    ...
    def _init_yolo(self, model_name: str):
        ...
        try:
            ...
            if torch.cuda.is_available():
                self.model.to('cuda')
                logger.info(f"YOLO使用GPU加速模式: {torch.cuda.get_device_name(0)}")
            else:
                logger.info("YOLO使用CPU模式")
    ...
    def _init_mediapipe(self):
        ...
        try:
            ...
            # 检查GPU状态
            if use_gpu:
                logger.info("MediaPipe使用GPU加速模式")
            else:
                logger.info("MediaPipe使用CPU模式")
    ...
    def run(self):
        ...
        # ========== 设置分辨率（降低可提升FPS） ==========
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)  # 设置宽度1280
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 960)  # 设置高度960
        cap.set(cv2.CAP_PROP_FPS, 60)  # 一些摄像头在低分辨率下支持更高帧率
